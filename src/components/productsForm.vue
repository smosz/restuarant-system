...
const fetchCategoryNames = async () => {
  try {
    ...
    availableCategories.value  = categoriesSnapshot.docs.map((doc) => doc.data().name);
  } catch (error) {
...
const registerNewCategory = () => {
  newCategory.value = newCategory.value.toUpperCase();
  if (newCategory.value) {
...
const registerProduct = async () => {
  try {
    ...
    console.log(originalCategoryObject)
