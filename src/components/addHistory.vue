...
<<<<<<< HEAD
    ...
    // Convert quantityChanges strings to actual arrays
    const productsWithParsedQuantityChanges = productsArray.map((product) => ({
      ...product,
      quantityChanges: JSON.parse(product.quantityChanges || '[]'),
    }));

   // Filter products with non-empty `quantityChanges` field
const filteredProducts = productsWithParsedQuantityChanges.filter(
  (product) => Array.isArray(product.quantityChanges) && product.quantityChanges.length > 0
);
